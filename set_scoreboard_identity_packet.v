...
pub fn (mut p SetScoreboardIdentityPacket) decode_payload(mut r Reader) ! {
	p.type = int(r.u8()!)
	count := int(r.read_varuint32()!)
	p.entries = []ScoreboardIdentityEntry{cap: count}
	for _ in 0 .. count {
...
pub fn (p &SetScoreboardIdentityPacket) encode_payload(mut w Writer) {
	w.u8(u8(p.type))
