...
fn (mut r Reader) read_skin_image() !SkinImage {
	return SkinImage{
		width:  r.le_u32()!
		height: r.le_u32()!
		data:   r.read_string()!
...
fn (mut w Writer) write_skin_image(img SkinImage) {
	w.le_u32(img.width)
	w.le_u32(img.height)
	w.write_string(img.data)
}

pub fn (mut r Reader) read_skin() !SkinData {
	mut s := SkinData{
	...
	s.persona_pieces = []PersonaSkinPiece{}
	...
	tint_count := r.le_u32()!
	s.piece_tint_colors = []PersonaPieceTintColor{}
...
pub fn (mut w Writer) write_skin(s SkinData) {
