...
pub fn (mut p NetworkChunkPublisherUpdatePacket) decode_payload(mut r Reader) ! {
	p.block_position = r.read_block_position()!
	p.radius = int(r.read_varuint32()!)
	count := int(r.le_u32()!)
	p.saved_chunks = []ChunkPosition{cap: count}
	for _ in 0 .. count {
		p.saved_chunks << r.read_chunk_position()!
	}
}

pub fn (p &NetworkChunkPublisherUpdatePacket) encode_payload(mut w Writer) {
