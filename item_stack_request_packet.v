...
fn read_stack_request_slot_info(mut r Reader) !StackRequestSlotInfo {
...
fn write_stack_request_slot_info(mut w Writer, s StackRequestSlotInfo) {
	w.write_full_container_name(s.container)
	w.u8(s.slot)
	w.write_varint32(s.stack_network_id)
}

fn read_stack_request_action(mut r Reader) !StackRequestAction {
	...
	match a.action_type {
		...
		stack_request_action_craft_recipe_auto {
			...
			a.ingredients = []ItemDescriptorCount{}
...
fn write_stack_request_action(mut w Writer, a StackRequestAction) {
	w.u8(a.action_type)
	match a.action_type {
		stack_request_action_take, stack_request_action_place, stack_request_action_place_in_container,
		stack_request_action_take_out_container {
...
fn read_item_stack_request_entry(mut r Reader) !ItemStackRequestEntry {
...
fn write_item_stack_request_entry(mut w Writer, e ItemStackRequestEntry) {
...
pub fn (mut p ItemStackRequestPacket) decode_payload(mut r Reader) ! {
...
pub fn (p &ItemStackRequestPacket) encode_payload(mut w Writer) {
