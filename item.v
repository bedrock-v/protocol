...
pub fn (mut r Reader) read_item_stack_without_stack_id() !ItemStack {
...
pub fn (mut w Writer) write_item_stack_without_stack_id(s ItemStack) {
	if s.id == 0 {
		w.write_varint32(0)
		return
	}
...
pub fn (mut r Reader) read_item_stack_wrapper() !ItemStackWrapper {
	id := int(r.read_varint32()!)
	if id == 0 {
		return ItemStackWrapper{
			stack_id:   0
			item_stack: item_stack_null()
		}
	}
	count := int(r.le_u16()!)
	meta := int(r.read_varuint32()!)
...
pub fn (mut w Writer) write_item_stack_wrapper(wrapper ItemStackWrapper) {
...
pub fn (mut r Reader) read_network_item_stack_descriptor() !ItemStackWrapper {
	id := int(r.le_i16()!)
	count := int(r.le_u16()!)
	meta := int(r.read_varuint32()!)
	has_net_id := r.bool()!
...
pub fn (mut w Writer) write_network_item_stack_descriptor(wrapper ItemStackWrapper) {
	s := wrapper.item_stack
	w.le_i16(i16(s.id))
	w.le_u16(u16(s.count))
	w.write_varuint32(u32(s.meta))
...
pub fn (mut r Reader) read_full_container_name() !FullContainerName {
...
pub fn (mut w Writer) write_full_container_name(name FullContainerName) {
