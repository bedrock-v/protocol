...
pub fn (mut p PositionTrackingDBClientRequestPacket) decode_payload(mut r Reader) ! {
	p.action = r.u8()!
	p.tracking_id = r.read_varint32()!
}

pub fn (p &PositionTrackingDBClientRequestPacket) encode_payload(mut w Writer) {
	w.u8(p.action)
