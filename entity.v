...
pub fn (mut r Reader) read_entity_link() !EntityLink {
	return EntityLink{
...
pub fn (mut w Writer) write_entity_link(link EntityLink) {
...
pub fn (mut r Reader) read_attribute_modifier() !AttributeModifier {
...
pub fn (mut w Writer) write_attribute_modifier(m AttributeModifier) {
	w.write_string(m.id)
...
pub fn (mut r Reader) read_update_attribute() !UpdateAttribute {
	min := r.le_f32()!
	max := r.le_f32()!
	current := r.le_f32()!
	...
	mut modifiers := []AttributeModifier{cap: modifier_count}
...
pub fn (mut w Writer) write_update_attribute(a UpdateAttribute) {
	w.le_f32(a.min)
	w.le_f32(a.max)
	w.le_f32(a.current)
