...
pub fn (mut p ClientboundDataDrivenUICloseScreenPacket) decode_payload(mut r Reader) ! {
	if r.bool()! {
		p.form_id = r.le_u32()!
	} else {
		p.form_id = none
...
pub fn (p &ClientboundDataDrivenUICloseScreenPacket) encode_payload(mut w Writer) {
	if id := p.form_id {
		w.bool(true)
		w.le_u32(id)
	} else {
