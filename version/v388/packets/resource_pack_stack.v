...
pub fn (mut p ResourcePackStackPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.behavior_packs = []packets_291.ResourcePackStackEntry{cap: serializer.prealloc(behavior_count)}
	for _ in 0 .. behavior_count {
		p.behavior_packs << packets_291.ResourcePackStackEntry.decode(mut r)!
	}
	resource_count := r.read_count()!
	p.resource_packs = []packets_291.ResourcePackStackEntry{cap: serializer.prealloc(resource_count)}
