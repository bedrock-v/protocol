...
pub mut:
	...
	height_map_data             [272]i8
	render_height_map_data_type HeightMapDataType
	render_height_map_data      [272]i8
	blob_id                     ?u64
}
...
pub fn SubChunkDataEntry.decode(mut r serializer.Reader) !SubChunkDataEntry {
	...
	height_map_data_raw := if has_height_map { decode_height_map(mut r)! } else { [
			272,
		]i8{} }
...
fn encode_opt_height_map(mut w serializer.Writer, present bool, data [272]i8) {
	w.bool(present)
	if present {
		for v in data {
			w.i8(v)
...
fn decode_height_map(mut r serializer.Reader) ![272]i8 {
	mut data := [272]i8{}
	for i in 0 .. 272 {
		data[i] = r.i8()!
	}
...
pub mut:
	cache_enabled  bool
	dimension_type i32
	center_pos     types_2168.SubChunkPos
...
pub fn (mut p SubChunkPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.center_pos = types_2168.SubChunkPos.decode(mut r)!
	count := r.read_count()!
