...
pub mut:
	...
	actions                  []types_2168.ItemStackRequestActionType
...
pub fn PerformItemStackRequestData.decode(mut r serializer.Reader) !PerformItemStackRequestData {
	...
	e.actions = []types_2168.ItemStackRequestActionType{cap: serializer.prealloc(act_count)}
	for _ in 0 .. act_count {
		e.actions << types_2168.ItemStackRequestActionType.decode(mut r)!
	}
...
pub mut:
	...
	player_block_actions     ?[]types_2168.PlayerBlockActionData
...
pub fn (mut p PlayerAuthInputPacket) decode_payload(mut r serializer.Reader) ! {
	...
	if r.bool()! {
		count := r.read_count()!
		mut actions := []types_2168.PlayerBlockActionData{cap: serializer.prealloc(count)}
		for _ in 0 .. count {
			actions << types_2168.PlayerBlockActionData.decode(mut r)!
		}
