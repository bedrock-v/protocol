...
pub mut:
	...
	rotation          [2]f32
	y_head_rotation   f32
	carried_item      types_662.NetworkItemStackDescriptor
	player_game_type  enums.GameType
	entity_data       []types_662.DataItem
	synced_properties types_662.PropertySyncData
	abilities_data    types_662.SerializedAbilitiesData
...
pub fn (mut p AddPlayerPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.carried_item = types_662.NetworkItemStackDescriptor.decode(mut r)!
	p.player_game_type = enums.GameType.decode(mut r)!
	{
		count := r.read_count()!
		p.entity_data = []types_662.DataItem{cap: serializer.prealloc(count)}
		for _ in 0 .. count {
			p.entity_data << types_662.DataItem.decode(mut r)!
		}
	}
	p.synced_properties = types_662.PropertySyncData.decode(mut r)!
	p.abilities_data = types_662.SerializedAbilitiesData.decode(mut r)!
	{
		count := r.read_count()!
