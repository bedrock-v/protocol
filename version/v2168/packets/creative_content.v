...
pub enum CreativeItemCategory as u8 {
	all               = 0
	construction      = 1
	nature            = 2
	equipment         = 3
...
pub fn CreativeItemCategory.decode(mut r serializer.Reader) !CreativeItemCategory {
	return unsafe { CreativeItemCategory(r.u8()!) }
}
...
pub fn (e CreativeItemGroup) encode(mut w serializer.Writer) {
...
pub fn (e CreativeItemData) encode(mut w serializer.Writer) {
	w.write_varuint32(e.creative_net_id)
	e.item_instance.encode(mut w)
	w.write_varuint32(e.group_id)
}
...
pub fn (mut p CreativeContentPacket) decode_payload(mut r serializer.Reader) ! {
	group_count := r.read_count()!
	p.groups = []CreativeItemGroup{cap: serializer.prealloc(group_count)}
	for _ in 0 .. group_count {
		p.groups << CreativeItemGroup.decode(mut r)!
	}
	content_count := r.read_count()!
	p.contents = []CreativeItemData{cap: serializer.prealloc(content_count)}
