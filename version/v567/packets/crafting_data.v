...
pub fn (mut p CraftingDataPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.crafting_data = []types.RecipeData{cap: serializer.prealloc(recipe_count)}
	for _ in 0 .. recipe_count {
		p.crafting_data << types.RecipeData.decode(mut r)!
	}
	potion_count := r.read_count()!
	p.potion_mix_data = []PotionMixData{cap: serializer.prealloc(potion_count)}
	...
	container_count := r.read_count()!
	p.container_mix_data = []ContainerMixData{cap: serializer.prealloc(container_count)}
	...
	p.material_reducers = []MaterialReducer{cap: serializer.prealloc(reducer_count)}
	for _ in 0 .. reducer_count {
		mut reducer := MaterialReducer{
			input_id: r.read_varint32()!
		}
		entry_count := r.read_count()!
		reducer.item_counts = []MaterialReducerEntry{cap: serializer.prealloc(entry_count)}
