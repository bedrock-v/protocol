...
pub fn (mut p CameraAimAssistPresetsPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.category_definitions = []types.CameraAimAssistCategory{cap: serializer.prealloc(category_count)}
	for _ in 0 .. category_count {
		p.category_definitions << types.CameraAimAssistCategory.decode(mut r)!
	}
	preset_count := r.read_count()!
	p.presets = []types.CameraAimAssistPresetDefinition{cap: serializer.prealloc(preset_count)}
