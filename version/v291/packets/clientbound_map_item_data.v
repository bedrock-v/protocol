...
pub fn (mut p ClientboundMapItemDataPacket) decode_payload(mut r serializer.Reader) ! {
	...
	if update_type & 0x4 != 0 {
		...
		p.tracked_objects = []types.MapTrackedObject{cap: serializer.prealloc(object_count)}
		for _ in 0 .. object_count {
			p.tracked_objects << types.MapTrackedObject.decode(mut r)!
		}
		decoration_count := r.read_count()!
		p.decorations = []types.MapDecoration{cap: serializer.prealloc(decoration_count)}
