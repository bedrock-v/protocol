...
pub mut:
	...
	trial                             bool
	movement_server_authoritative     bool
	current_tick                      i64
...
pub fn (p &StartGamePacket) encode_payload(mut w serializer.Writer) {
	...
	w.bool(p.trial)
	w.bool(p.movement_server_authoritative)
	w.le_i64(p.current_tick)
	w.write_varint32(p.enchantment_seed)
	types_388.write_nbt_root(mut w, p.block_palette)
...
pub fn (mut p StartGamePacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.trial = r.bool()!
	p.movement_server_authoritative = r.bool()!
	p.current_tick = r.le_i64()!
	p.enchantment_seed = r.read_varint32()!
	p.block_palette = types_388.read_nbt_root(mut r)!
