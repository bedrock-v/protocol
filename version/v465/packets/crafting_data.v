...
pub fn (mut p CraftingDataPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.crafting_data = []types_431.RecipeData{cap: serializer.prealloc(recipe_count)}
	for _ in 0 .. recipe_count {
		p.crafting_data << types_431.RecipeData.decode(mut r)!
	}
	potion_count := r.read_count()!
	p.potion_mix_data = []types_407.PotionMixData{cap: serializer.prealloc(potion_count)}
	for _ in 0 .. potion_count {
		p.potion_mix_data << types_407.PotionMixData.decode(mut r)!
	}
	container_count := r.read_count()!
	p.container_mix_data = []types_407.ContainerMixData{cap: serializer.prealloc(container_count)}
	for _ in 0 .. container_count {
		p.container_mix_data << types_407.ContainerMixData.decode(mut r)!
	}
	reducer_count := r.read_count()!
	p.material_reducers = []types.MaterialReducer{cap: serializer.prealloc(reducer_count)}
