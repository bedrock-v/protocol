module types

import protocol.serializer

pub struct AchievementAwardedEventData {
pub mut:
	achievement_id i32
}

pub struct EntityInteractEventData {
pub mut:
	interaction_type      i32
	legacy_entity_type_id i32
	variant               i32
	palette_color         u8
}

pub struct PortalBuiltEventData {
pub mut:
	dimension_id i32
}

pub struct PortalUsedEventData {
pub mut:
	from_dimension_id i32
	to_dimension_id   i32
}

pub struct MobKilledEventData {
pub mut:
	killer_unique_entity_id i64
	victim_unique_entity_id i64
	killer_entity_type      i32
	entity_damage_cause     i32
	villager_trade_tier     i32
	villager_display_name   string
}

pub struct CauldronUsedEventData {
pub mut:
	potion_id  i32
	color      i32
	fill_level i32
}

pub struct PlayerDiedEventData {
pub mut:
	attacker_entity_id  i32
	attacker_variant    i32
	entity_damage_cause i32
	in_raid             bool
}

pub struct BossKilledEventData {
pub mut:
	boss_unique_entity_id i64
	player_party_size     i32
	boss_entity_type      i32
}

pub struct AgentCommandEventData {
pub mut:
	result     i32
	data_value i32
	command    string
	data_key   string
	output     string
}

pub struct AgentCreatedEventData {}

pub struct PatternRemovedEventData {
pub mut:
	item_id       i32
	aux_value     i32
	patterns_size i32
	pattern_index i32
	pattern_color i32
}

pub struct SlashCommandExecutedEventData {
pub mut:
	success_count   i32
	error_count     i32
	command_name    string
	output_messages []string
}

pub struct FishBucketedEventData {
pub mut:
	pattern              i32
	preset               i32
	bucketed_entity_type i32
	release_event        bool
}

pub struct MobBornEventData {
pub mut:
	entity_type i32
	variant     i32
	color       u8
}

pub struct PetDiedEventData {
pub mut:
	owner_killed            bool
	killer_unique_entity_id i64
	pet_unique_entity_id    i64
	entity_damage_cause     i32
	pet_entity_type         i32
}

pub struct CauldronInteractEventData {
pub mut:
	block_interaction_type i32
	item_id                i32
}

pub struct ComposterInteractEventData {
pub mut:
	block_interaction_type i32
	item_id                i32
}

pub struct BellUsedEventData {
pub mut:
	item_id i32
}

pub struct EntityDefinitionTriggerEventData {
pub mut:
	event_name string
}

pub struct RaidUpdateEventData {
pub mut:
	current_wave i32
	total_waves  i32
	winner       bool
}

pub struct MovementAnomalyEventData {
pub mut:
	event_type             i8
	cheating_score         f32
	average_position_delta f32
	total_position_delta   f32
	min_position_delta     f32
	max_position_delta     f32
}

pub struct MovementCorrectedEventData {
pub mut:
	position_delta     f32
	cheating_score     f32
	score_threshold    f32
	distance_threshold f32
	duration_threshold i32
}

pub struct ExtractHoneyEventData {}

pub struct TargetBlockHitEventData {
pub mut:
	redstone_level i32
}

pub struct PiglinBarterEventData {
pub mut:
	item_runtime_id  i32
	targeting_player bool
}

pub struct CopperWaxedOrUnwaxedEventData {
pub mut:
	block_runtime_id i32
}

pub struct CodeBuilderActionEventData {
pub mut:
	action string
}

pub struct CodeBuilderScoreboardEventData {
pub mut:
	objective_name string
	score          i32
}

pub struct StriderRiddenInLavaInOverworldEventData {}

pub struct SneakCloseToSculkSensorEventData {}

pub struct CarefulRestorationEventData {}

pub type EventData = AchievementAwardedEventData
	| AgentCommandEventData
	| AgentCreatedEventData
	| BellUsedEventData
	| BossKilledEventData
	| CarefulRestorationEventData
	| CauldronInteractEventData
	| CauldronUsedEventData
	| CodeBuilderActionEventData
	| CodeBuilderScoreboardEventData
	| ComposterInteractEventData
	| CopperWaxedOrUnwaxedEventData
	| EntityDefinitionTriggerEventData
	| EntityInteractEventData
	| ExtractHoneyEventData
	| FishBucketedEventData
	| MobBornEventData
	| MobKilledEventData
	| MovementAnomalyEventData
	| MovementCorrectedEventData
	| PatternRemovedEventData
	| PetDiedEventData
	| PiglinBarterEventData
	| PlayerDiedEventData
	| PortalBuiltEventData
	| PortalUsedEventData
	| RaidUpdateEventData
	| SlashCommandExecutedEventData
	| SneakCloseToSculkSensorEventData
	| StriderRiddenInLavaInOverworldEventData
	| TargetBlockHitEventData

pub fn (t EventData) id() i32 {
	return match t {
		AchievementAwardedEventData { i32(0) }
		EntityInteractEventData { i32(1) }
		PortalBuiltEventData { i32(2) }
		PortalUsedEventData { i32(3) }
		MobKilledEventData { i32(4) }
		CauldronUsedEventData { i32(5) }
		PlayerDiedEventData { i32(6) }
		BossKilledEventData { i32(7) }
		AgentCommandEventData { i32(8) }
		AgentCreatedEventData { i32(9) }
		PatternRemovedEventData { i32(10) }
		SlashCommandExecutedEventData { i32(11) }
		FishBucketedEventData { i32(12) }
		MobBornEventData { i32(13) }
		PetDiedEventData { i32(14) }
		CauldronInteractEventData { i32(15) }
		ComposterInteractEventData { i32(16) }
		BellUsedEventData { i32(17) }
		EntityDefinitionTriggerEventData { i32(18) }
		RaidUpdateEventData { i32(19) }
		MovementAnomalyEventData { i32(20) }
		MovementCorrectedEventData { i32(21) }
		ExtractHoneyEventData { i32(22) }
		TargetBlockHitEventData { i32(23) }
		PiglinBarterEventData { i32(24) }
		CopperWaxedOrUnwaxedEventData { i32(25) }
		CodeBuilderActionEventData { i32(26) }
		CodeBuilderScoreboardEventData { i32(27) }
		StriderRiddenInLavaInOverworldEventData { i32(28) }
		SneakCloseToSculkSensorEventData { i32(29) }
		CarefulRestorationEventData { i32(30) }
	}
}

pub fn (t EventData) encode_payload(mut w serializer.Writer) {
	match t {
		AchievementAwardedEventData {
			w.write_varint32(t.achievement_id)
		}
		EntityInteractEventData {
			w.write_varint32(t.interaction_type)
			w.write_varint32(t.legacy_entity_type_id)
			w.write_varint32(t.variant)
			w.u8(t.palette_color)
		}
		PortalBuiltEventData {
			w.write_varint32(t.dimension_id)
		}
		PortalUsedEventData {
			w.write_varint32(t.from_dimension_id)
			w.write_varint32(t.to_dimension_id)
		}
		MobKilledEventData {
			w.write_varint64(t.killer_unique_entity_id)
			w.write_varint64(t.victim_unique_entity_id)
			w.write_varint32(t.killer_entity_type)
			w.write_varint32(t.entity_damage_cause)
			w.write_varint32(t.villager_trade_tier)
			w.write_string(t.villager_display_name)
		}
		CauldronUsedEventData {
			w.write_varuint32(u32(t.potion_id))
			w.write_varint32(t.color)
			w.write_varint32(t.fill_level)
		}
		PlayerDiedEventData {
			w.write_varint32(t.attacker_entity_id)
			w.write_varint32(t.attacker_variant)
			w.write_varint32(t.entity_damage_cause)
			w.bool(t.in_raid)
		}
		BossKilledEventData {
			w.write_varint64(t.boss_unique_entity_id)
			w.write_varint32(t.player_party_size)
			w.write_varint32(t.boss_entity_type)
		}
		AgentCommandEventData {
			w.write_varint32(t.result)
			w.write_varint32(t.data_value)
			w.write_string(t.command)
			w.write_string(t.data_key)
			w.write_string(t.output)
		}
		AgentCreatedEventData {}
		PatternRemovedEventData {
			w.write_varint32(t.item_id)
			w.write_varint32(t.aux_value)
			w.write_varint32(t.patterns_size)
			w.write_varint32(t.pattern_index)
			w.write_varint32(t.pattern_color)
		}
		SlashCommandExecutedEventData {
			w.write_varint32(t.success_count)
			w.write_varint32(t.error_count)
			w.write_string(t.command_name)
			w.write_string(t.output_messages.join(';'))
		}
		FishBucketedEventData {
			w.write_varint32(t.pattern)
			w.write_varint32(t.preset)
			w.write_varint32(t.bucketed_entity_type)
			w.bool(t.release_event)
		}
		MobBornEventData {
			w.write_varint32(t.entity_type)
			w.write_varint32(t.variant)
			w.u8(t.color)
		}
		PetDiedEventData {
			w.bool(t.owner_killed)
			w.write_varint64(t.killer_unique_entity_id)
			w.write_varint64(t.pet_unique_entity_id)
			w.write_varint32(t.entity_damage_cause)
			w.write_varint32(t.pet_entity_type)
		}
		CauldronInteractEventData {
			w.write_varint32(t.block_interaction_type)
			w.write_varint32(t.item_id)
		}
		ComposterInteractEventData {
			w.write_varint32(t.block_interaction_type)
			w.write_varint32(t.item_id)
		}
		BellUsedEventData {
			w.write_varint32(t.item_id)
		}
		EntityDefinitionTriggerEventData {
			w.write_string(t.event_name)
		}
		RaidUpdateEventData {
			w.write_varint32(t.current_wave)
			w.write_varint32(t.total_waves)
			w.bool(t.winner)
		}
		MovementAnomalyEventData {
			w.i8(t.event_type)
			w.le_f32(t.cheating_score)
			w.le_f32(t.average_position_delta)
			w.le_f32(t.total_position_delta)
			w.le_f32(t.min_position_delta)
			w.le_f32(t.max_position_delta)
		}
		MovementCorrectedEventData {
			w.le_f32(t.position_delta)
			w.le_f32(t.cheating_score)
			w.le_f32(t.score_threshold)
			w.le_f32(t.distance_threshold)
			w.write_varint32(t.duration_threshold)
		}
		ExtractHoneyEventData {}
		TargetBlockHitEventData {
			w.write_varint32(t.redstone_level)
		}
		PiglinBarterEventData {
			w.write_varint32(t.item_runtime_id)
			w.bool(t.targeting_player)
		}
		CopperWaxedOrUnwaxedEventData {
			w.write_varint32(t.block_runtime_id)
		}
		CodeBuilderActionEventData {
			w.write_string(t.action)
		}
		CodeBuilderScoreboardEventData {
			w.write_string(t.objective_name)
			w.write_varint32(t.score)
		}
		StriderRiddenInLavaInOverworldEventData {}
		SneakCloseToSculkSensorEventData {}
		CarefulRestorationEventData {}
	}
}

pub fn EventData.decode_payload(id i32, mut r serializer.Reader) !EventData {
	match id {
		0 {
			return AchievementAwardedEventData{
				achievement_id: r.read_varint32()!
			}
		}
		1 {
			return EntityInteractEventData{
				interaction_type:      r.read_varint32()!
				legacy_entity_type_id: r.read_varint32()!
				variant:               r.read_varint32()!
				palette_color:         r.u8()!
			}
		}
		2 {
			return PortalBuiltEventData{
				dimension_id: r.read_varint32()!
			}
		}
		3 {
			return PortalUsedEventData{
				from_dimension_id: r.read_varint32()!
				to_dimension_id:   r.read_varint32()!
			}
		}
		4 {
			return MobKilledEventData{
				killer_unique_entity_id: r.read_varint64()!
				victim_unique_entity_id: r.read_varint64()!
				killer_entity_type:      r.read_varint32()!
				entity_damage_cause:     r.read_varint32()!
				villager_trade_tier:     r.read_varint32()!
				villager_display_name:   r.read_string()!
			}
		}
		5 {
			return CauldronUsedEventData{
				potion_id:  r.read_varint32()!
				color:      r.read_varint32()!
				fill_level: r.read_varint32()!
			}
		}
		6 {
			return PlayerDiedEventData{
				attacker_entity_id:  r.read_varint32()!
				attacker_variant:    r.read_varint32()!
				entity_damage_cause: r.read_varint32()!
				in_raid:             r.bool()!
			}
		}
		7 {
			return BossKilledEventData{
				boss_unique_entity_id: r.read_varint64()!
				player_party_size:     r.read_varint32()!
				boss_entity_type:      r.read_varint32()!
			}
		}
		8 {
			return AgentCommandEventData{
				result:     r.read_varint32()!
				data_value: r.read_varint32()!
				command:    r.read_string()!
				data_key:   r.read_string()!
				output:     r.read_string()!
			}
		}
		9 {
			return AgentCreatedEventData{}
		}
		10 {
			return PatternRemovedEventData{
				item_id:       r.read_varint32()!
				aux_value:     r.read_varint32()!
				patterns_size: r.read_varint32()!
				pattern_index: r.read_varint32()!
				pattern_color: r.read_varint32()!
			}
		}
		11 {
			return SlashCommandExecutedEventData{
				success_count:   r.read_varint32()!
				error_count:     r.read_varint32()!
				command_name:    r.read_string()!
				output_messages: r.read_string()!.split(';')
			}
		}
		12 {
			return FishBucketedEventData{
				pattern:              r.read_varint32()!
				preset:               r.read_varint32()!
				bucketed_entity_type: r.read_varint32()!
				release_event:        r.bool()!
			}
		}
		13 {
			return MobBornEventData{
				entity_type: r.read_varint32()!
				variant:     r.read_varint32()!
				color:       r.u8()!
			}
		}
		14 {
			return PetDiedEventData{
				owner_killed:            r.bool()!
				killer_unique_entity_id: r.read_varint64()!
				pet_unique_entity_id:    r.read_varint64()!
				entity_damage_cause:     r.read_varint32()!
				pet_entity_type:         r.read_varint32()!
			}
		}
		15 {
			return CauldronInteractEventData{
				block_interaction_type: r.read_varint32()!
				item_id:                r.read_varint32()!
			}
		}
		16 {
			return ComposterInteractEventData{
				block_interaction_type: r.read_varint32()!
				item_id:                r.read_varint32()!
			}
		}
		17 {
			return BellUsedEventData{
				item_id: r.read_varint32()!
			}
		}
		18 {
			return EntityDefinitionTriggerEventData{
				event_name: r.read_string()!
			}
		}
		19 {
			return RaidUpdateEventData{
				current_wave: r.read_varint32()!
				total_waves:  r.read_varint32()!
				winner:       r.bool()!
			}
		}
		20 {
			return MovementAnomalyEventData{
				event_type:             r.i8()!
				cheating_score:         r.le_f32()!
				average_position_delta: r.le_f32()!
				total_position_delta:   r.le_f32()!
				min_position_delta:     r.le_f32()!
				max_position_delta:     r.le_f32()!
			}
		}
		21 {
			return MovementCorrectedEventData{
				position_delta:     r.le_f32()!
				cheating_score:     r.le_f32()!
				score_threshold:    r.le_f32()!
				distance_threshold: r.le_f32()!
				duration_threshold: r.read_varint32()!
			}
		}
		22 {
			return ExtractHoneyEventData{}
		}
		23 {
			return TargetBlockHitEventData{
				redstone_level: r.read_varint32()!
			}
		}
		24 {
			return PiglinBarterEventData{
				item_runtime_id:  r.read_varint32()!
				targeting_player: r.bool()!
			}
		}
		25 {
			return CopperWaxedOrUnwaxedEventData{
				block_runtime_id: r.read_varint32()!
			}
		}
		26 {
			return CodeBuilderActionEventData{
				action: r.read_string()!
			}
		}
		27 {
			return CodeBuilderScoreboardEventData{
				objective_name: r.read_string()!
				score:          r.read_varint32()!
			}
		}
		28 {
			return StriderRiddenInLavaInOverworldEventData{}
		}
		29 {
			return SneakCloseToSculkSensorEventData{}
		}
		30 {
			return CarefulRestorationEventData{}
		}
		else {
			return error('unknown event data type: ${id}')
		}
	}
}
