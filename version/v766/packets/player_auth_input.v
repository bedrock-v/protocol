...
pub mut:
	action_type types_729.ItemStackRequestActionType
	amount      i8
	source      types_729.ItemStackRequestSlotInfo
	destination types_729.ItemStackRequestSlotInfo
}
...
pub fn ActionsEntry.decode(mut r serializer.Reader) !ActionsEntry {
	return ActionsEntry{
		action_type: types_729.ItemStackRequestActionType.decode(mut r)!
		amount:      r.i8()!
		source:      types_729.ItemStackRequestSlotInfo.decode(mut r)!
		destination: types_729.ItemStackRequestSlotInfo.decode(mut r)!
	}
}
...
pub fn PerformItemStackRequestData.decode(mut r serializer.Reader) !PerformItemStackRequestData {
	...
	t.actions = []ActionsEntry{cap: serializer.prealloc(action_count)}
	for _ in 0 .. action_count {
		t.actions << ActionsEntry.decode(mut r)!
	}
	filter_count := r.read_count()!
	t.strings_to_filter = []string{cap: serializer.prealloc(filter_count)}
...
pub mut:
	...
	client_tick              u64
	velocity                 [3]f32
	item_use_transaction     ?types_712.PackedItemUseLegacyInventoryTransaction
...
pub fn (p &PlayerAuthInputPacket) encode_payload(mut w serializer.Writer) {
	...
	write_varuint128(mut w, p.input_data, p.input_data_hi)
	...
	if p.input_data & input_flag_perform_item_interaction != 0 {
		tx := p.item_use_transaction or { types_712.PackedItemUseLegacyInventoryTransaction{} }
		tx.encode(mut w)
	}
...
pub fn (mut p PlayerAuthInputPacket) decode_payload(mut r serializer.Reader) ! {
	...
	if p.input_data & input_flag_perform_item_interaction != 0 {
		p.item_use_transaction = types_712.PackedItemUseLegacyInventoryTransaction.decode(mut r)!
	}
	if p.input_data & input_flag_perform_item_stack_request != 0 {
...
// input_data is a u128 varint on the wire, carried here as a low and a high u64 half
