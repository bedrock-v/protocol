...
import protocol.version.v594.types
...
pub fn (mut p AvailableCommandsPacket) decode_payload(mut r serializer.Reader) ! {
	...
	sub_command_value_count := r.read_count()!
	p.sub_command_values = []string{cap: serializer.prealloc(sub_command_value_count)}
	...
	sub_command_count := r.read_count()!
	p.sub_commands = []types.ChainedSubCommandData{cap: serializer.prealloc(sub_command_count)}
