...
import protocol.version.v340.types

pub struct InventoryContentPacket {
pub mut:
	container_id u32
	contents     []types.ItemData
}
...
pub fn (mut p InventoryContentPacket) decode_payload(mut r serializer.Reader) ! {
	...
	count := r.read_count()!
	p.contents = []types.ItemData{cap: serializer.prealloc(count)}
	for _ in 0 .. count {
		p.contents << types.ItemData.decode(mut r)!
	}
}
