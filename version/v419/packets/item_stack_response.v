...
pub mut:
	result     ItemStackResponseStatus
	request_id i32
	containers []types_407.ItemStackResponseContainer
}
...
pub fn ItemStackResponse.decode(mut r serializer.Reader) !ItemStackResponse {
	...
	t.result = unsafe { ItemStackResponseStatus(r.u8()!) }
	t.request_id = r.read_varint32()!
	if t.result != .ok {
		return t
	}
	count := r.read_count()!
	t.containers = []types_407.ItemStackResponseContainer{cap: serializer.prealloc(count)}
	for _ in 0 .. count {
		t.containers << types_407.ItemStackResponseContainer.decode(mut r)!
	}
	return t
}
