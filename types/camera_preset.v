...
pub mut:
	...
	player_effects                    ?bool
	aim_assist_preset                 ?CameraAimAssistPreset
	control_scheme                    ?enums.ControlScheme
	apply_inherited_starting_rotation bool
	starting_rotation                 ?[2]f32
}
...
pub fn (t CameraPreset) encode(mut w serializer.Writer) {
	...
	if v := t.control_scheme {
		w.bool(true)
		v.encode(mut w)
	} else {
		w.bool(false)
	}
	w.bool(t.apply_inherited_starting_rotation)
	if v := t.starting_rotation {
		w.bool(true)
		w.le_f32(v[0])
		w.le_f32(v[1])
	} else {
...
pub fn CameraPreset.decode(mut r serializer.Reader) !CameraPreset {
	...
	if r.bool()! {
		t.aim_assist_preset = CameraAimAssistPreset.decode(mut r)!
	}
	if r.bool()! {
		t.control_scheme = enums.ControlScheme.decode(mut r)!
	}
	t.apply_inherited_starting_rotation = r.bool()!
	if r.bool()! {
		t.starting_rotation = [r.le_f32()!, r.le_f32()!]!
	}
	return t
}
