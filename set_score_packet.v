...
pub fn (mut p SetScorePacket) decode_payload(mut r Reader) ! {
	p.type = int(r.u8()!)
	count := int(r.read_varuint32()!)
	p.entries = []ScorePacketEntry{cap: count}
	for _ in 0 .. count {
		...
		if p.type != set_score_type_remove {
			entry.type = int(r.u8()!)
			match entry.type {
				score_entry_type_player, score_entry_type_entity {
					entry.actor_unique_id = r.read_actor_unique_id()!
				}
				score_entry_type_fake_player {
...
pub fn (p &SetScorePacket) encode_payload(mut w Writer) {
	w.u8(u8(p.type))
