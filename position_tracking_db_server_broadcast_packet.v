...
pub fn (mut p PositionTrackingDBServerBroadcastPacket) decode_payload(mut r Reader) ! {
	p.action = r.u8()!
	p.tracking_id = r.read_varint32()!
	p.nbt = r.read_nbt_compound_root()!
}

pub fn (p &PositionTrackingDBServerBroadcastPacket) encode_payload(mut w Writer) {
	w.u8(p.action)
