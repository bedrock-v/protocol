...
fn read_biome_surface_material(mut r Reader) !BiomeSurfaceMaterial {
...
fn write_biome_surface_material(mut w Writer, m BiomeSurfaceMaterial) {
...
fn read_noise_descriptor(mut r Reader) !NoiseDescriptor {
...
fn write_noise_descriptor(mut w Writer, n NoiseDescriptor) {
...
fn read_noise_block_specifier(mut r Reader) !NoiseBlockSpecifier {
...
fn write_noise_block_specifier(mut w Writer, s NoiseBlockSpecifier) {
...
fn read_noise_gradient_surface(mut r Reader) !BiomeNoiseGradientSurface {
...
fn write_noise_gradient_surface(mut w Writer, g BiomeNoiseGradientSurface) {
...
fn read_mesa_surface(mut r Reader) !BiomeMesaSurface {
...
fn write_mesa_surface(mut w Writer, m BiomeMesaSurface) {
...
fn read_capped_surface(mut r Reader) !BiomeCappedSurface {
...
fn write_capped_surface(mut w Writer, c BiomeCappedSurface) {
...
fn read_surface_builder(mut r Reader) !BiomeSurfaceBuilder {
...
fn write_surface_builder(mut w Writer, b BiomeSurfaceBuilder) {
...
fn read_biome_weight_list(mut r Reader) ![]BiomeWeight {
	count := r.read_varuint32()!
	mut out := []BiomeWeight{}
	for _ in 0 .. count {
		out << BiomeWeight{
...
fn write_biome_weight_list(mut w Writer, list []BiomeWeight) {
...
fn read_conditional_transformation_list(mut r Reader) ![]BiomeConditionalTransformation {
	count := r.read_varuint32()!
...
fn write_conditional_transformation_list(mut w Writer, list []BiomeConditionalTransformation) {
	w.write_varuint32(u32(list.len))
	for c in list {
...
fn read_overworld_rules(mut r Reader) !BiomeOverworldRules {
...
fn write_overworld_rules(mut w Writer, o BiomeOverworldRules) {
...
fn read_replacement_data_list(mut r Reader) ![]BiomeReplacementData {
	count := r.read_varuint32()!
...
fn write_replacement_data_list(mut w Writer, list []BiomeReplacementData) {
	w.write_varuint32(u32(list.len))
	for d in list {
		w.le_i16(d.biome)
...
fn read_biome_coordinate(mut r Reader) !BiomeCoordinate {
...
fn write_biome_coordinate(mut w Writer, c BiomeCoordinate) {
...
fn read_scatter_parameter(mut r Reader) !BiomeScatterParameter {
...
fn write_scatter_parameter(mut w Writer, s BiomeScatterParameter) {
...
fn read_consolidated_feature_list(mut r Reader) ![]BiomeConsolidatedFeature {
	count := r.read_varuint32()!
...
fn write_consolidated_feature_list(mut w Writer, list []BiomeConsolidatedFeature) {
	w.write_varuint32(u32(list.len))
	for f in list {
...
fn read_element_data_list(mut r Reader) ![]BiomeElementData {
	count := r.read_varuint32()!
...
fn write_element_data_list(mut w Writer, list []BiomeElementData) {
	w.write_varuint32(u32(list.len))
	for e in list {
...
fn read_chunk_generation(mut r Reader) !BiomeChunkGeneration {
...
fn write_chunk_generation(mut w Writer, g BiomeChunkGeneration) {
	if g.has_climate {
		w.bool(true)
...
fn read_biome_definition(mut r Reader) !BiomeDefinition {
...
fn write_biome_definition(mut w Writer, d BiomeDefinition) {
...
pub fn (mut p BiomeDefinitionListPacket) decode_payload(mut r Reader) ! {
...
pub fn (p &BiomeDefinitionListPacket) encode_payload(mut w Writer) {
