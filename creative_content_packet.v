...
pub fn (mut p CreativeContentPacket) decode_payload(mut r Reader) ! {
	group_count := int(r.read_varuint32()!)
	p.groups = []CreativeGroupEntry{}
	for _ in 0 .. group_count {
		p.groups << r.read_creative_group_entry()!
	}
	item_count := int(r.read_varuint32()!)
	p.items = []CreativeItemEntry{}
	for _ in 0 .. item_count {
		p.items << r.read_creative_item_entry()!
	}
}

pub fn (p &CreativeContentPacket) encode_payload(mut w Writer) {
