...
pub mut:
	...
	vehicle_rotation         Vector2
	client_predicted_vehicle i64
	analogue_move_vector     Vector2
	camera_orientation       Vector3
	raw_move_vector          Vector2
}
...
fn read_input_bitset(mut r Reader) ![]u8 {
	mut b := []u8{}
	for {
		x := r.u8()!
		b << x
...
fn write_input_bitset(mut w Writer, b []u8) {
	if b.len == 0 {
		w.u8(0)
		return
	}
...
pub fn (mut p PlayerAuthInputPacket) decode_payload(mut r Reader) ! {
	p.pitch = r.le_f32()!
	p.yaw = r.le_f32()!
...
pub fn (p &PlayerAuthInputPacket) encode_payload(mut w Writer) {
	w.le_f32(p.pitch)
	w.le_f32(p.yaw)
