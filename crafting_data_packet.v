...
fn read_recipe_unlock_requirement(mut r Reader) !RecipeUnlockRequirement {
	mut u := RecipeUnlockRequirement{
		context: r.u8()!
	}
	if u.context == recipe_unlock_context_none {
		count := r.read_varuint32()!
		u.ingredients = []ItemDescriptorCount{}
		for _ in 0 .. count {
...
fn write_recipe_unlock_requirement(mut w Writer, u RecipeUnlockRequirement) {
	w.u8(u.context)
	if u.context == recipe_unlock_context_none {
...
fn read_descriptor_list(mut r Reader) ![]ItemDescriptorCount {
	count := r.read_varuint32()!
	mut out := []ItemDescriptorCount{}
	for _ in 0 .. count {
...
fn read_recipe_output(mut r Reader) ![]ItemStack {
	count := r.read_varuint32()!
	mut out := []ItemStack{}
	for _ in 0 .. count {
...
fn write_recipe_output(mut w Writer, out []ItemStack) {
...
fn read_recipe(mut r Reader) !Recipe {
	mut rec := Recipe{
...
fn write_recipe(mut w Writer, rec Recipe) {
...
pub fn (mut p CraftingDataPacket) decode_payload(mut r Reader) ! {
...
pub fn (p &CraftingDataPacket) encode_payload(mut w Writer) {
