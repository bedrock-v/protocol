...
pub mut:
	...
	dimension         i8
	is_locked         bool
	map_origin        types.NetworkBlockPosition
	creation_map_ids  ?[]types.ActorUniqueID
	scale             ?i8
...
pub fn (mut p ClientBoundMapItemDataPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.dimension = r.i8()!
	p.is_locked = r.bool()!
	p.map_origin = types.NetworkBlockPosition.decode(mut r)!
	if r.bool()! {
		count := r.read_count()!
		mut ids := []types.ActorUniqueID{cap: serializer.prealloc(count)}
		for _ in 0 .. count {
			ids << types.ActorUniqueID.decode(mut r)!
		}
