...
pub fn (t GatheringJoinInfo) encode(mut w serializer.Writer) {
	...
	encode_opt_string(mut w, t.experience_name)
	w.write_string(t.experience_world_id)
	encode_opt_string(mut w, t.experience_world_name)
...
pub fn GatheringJoinInfo.decode(mut r serializer.Reader) !GatheringJoinInfo {
	...
	t.unknown1 = types.Uuid.decode(mut r)!
	t.unknown2 = types.Uuid.decode(mut r)!
...
pub fn (t PresenceInfo) encode(mut w serializer.Writer) {
	encode_opt_string(mut w, t.rich_presence_id)
}
...
fn encode_opt_string(mut w serializer.Writer, v ?string) {
	if val := v {
		w.bool(true)
		w.write_string(val)
	} else {
...
pub fn (mut p StartGamePacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.is_trial = r.bool()!
	p.movement_settings = types.SyncedPlayerMovementSettings.decode(mut r)!
	...
	p.world_template_id = types.Uuid.decode(mut r)!
	p.server_enabled_client_side_generation = r.bool()!
	p.block_network_ids_are_hashes = r.bool()!
	p.network_permissions = types.NetworkPermissions.decode(mut r)!
	if r.bool()! {
