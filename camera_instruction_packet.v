...
fn read_camera_set(mut r Reader) !CameraSetInstruction {
...
fn write_camera_set(mut w Writer, s CameraSetInstruction) {
	w.le_u32(s.preset)
	if e := s.ease {
		w.bool(true)
		w.u8(e.type)
...
fn read_camera_fade(mut r Reader) !CameraFadeInstruction {
...
fn write_camera_fade(mut w Writer, f CameraFadeInstruction) {
	if t := f.time {
		w.bool(true)
		w.le_f32(t.fade_in)
		w.le_f32(t.stay)
...
pub fn (mut p CameraInstructionPacket) decode_payload(mut r Reader) ! {
	if r.bool()! {
...
pub fn (p &CameraInstructionPacket) encode_payload(mut w Writer) {
	if s := p.set {
		w.bool(true)
