...
pub fn (mut r Reader) read_vector3() !Vector3 {
	return Vector3{
		x: r.le_f32()!
		y: r.le_f32()!
		z: r.le_f32()!
...
pub fn (mut w Writer) write_vector3(v Vector3) {
	w.le_f32(v.x)
	w.le_f32(v.y)
	w.le_f32(v.z)
}

pub fn (mut r Reader) read_vector2() !Vector2 {
	return Vector2{
		x: r.le_f32()!
		y: r.le_f32()!
	}
...
pub fn (mut w Writer) write_vector2(v Vector2) {
	w.le_f32(v.x)
	w.le_f32(v.y)
}

pub fn (mut r Reader) read_block_position() !BlockPosition {
...
pub fn (mut w Writer) write_block_position(v BlockPosition) {
...
pub fn (mut r Reader) read_uuid() !UUID {
	mut p1 := r.read_raw(8)!
	mut p2 := r.read_raw(8)!
	p1.reverse_in_place()
	p2.reverse_in_place()
	p1 << p2
	return uuid_from_bytes(p1)
}

pub fn (mut w Writer) write_uuid(u UUID) {
	b := u.to_array()
	mut p1 := b[0..8].clone()
	mut p2 := b[8..16].clone()
	p1.reverse_in_place()
