...
pub fn (mut p CraftingDataPacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.crafting_data = []types_361.RecipeData{cap: serializer.prealloc(recipe_count)}
	for _ in 0 .. recipe_count {
		p.crafting_data << types_361.RecipeData.decode(mut r)!
	}
	potion_mix_count := r.read_count()!
	p.potion_mix_data = []types.PotionMixData{cap: serializer.prealloc(potion_mix_count)}
	for _ in 0 .. potion_mix_count {
		p.potion_mix_data << types.PotionMixData.decode(mut r)!
	}
	container_mix_count := r.read_count()!
	p.container_mix_data = []types.ContainerMixData{cap: serializer.prealloc(container_mix_count)}
