...
pub enum ChatRestrictionLevel as u8 {
	@none    = 0
	dropped  = 1
	disabled = 2
}
...
fn (p &StartGamePacket) encode_level_settings(mut w serializer.Writer) {
	...
	w.u8(u8(p.chat_restriction_level))
...
fn (mut p StartGamePacket) decode_level_settings(mut r serializer.Reader) ! {
	...
	p.chat_restriction_level = unsafe { ChatRestrictionLevel(r.u8()!) }
...
pub fn (mut p StartGamePacket) decode_payload(mut r serializer.Reader) ! {
	...
	property_count := int(r.read_varuint32()!)
	p.block_properties = []BlockPropertyData{cap: property_count}
