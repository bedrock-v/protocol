...
pub mut:
	inventory_id        u32
	slots               []types_975.NetworkItemStackDescriptorV2
	container_name_data types_944.FullContainerName
	storage_item        types_975.NetworkItemStackDescriptorV2
}
...
pub fn (mut p InventoryContentPacket) decode_payload(mut r serializer.Reader) ! {
	...
	{
		count := r.read_count()!
		p.slots = []types_975.NetworkItemStackDescriptorV2{cap: serializer.prealloc(count)}
		for _ in 0 .. count {
			p.slots << types_975.NetworkItemStackDescriptorV2.decode(mut r)!
		}
	}
	p.container_name_data = types_944.FullContainerName.decode(mut r)!
	p.storage_item = types_975.NetworkItemStackDescriptorV2.decode(mut r)!
}
