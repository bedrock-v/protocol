...
import protocol.version.v419.types
...
pub mut:
	...
	experiments                       []types.ExperimentData
	...
	trial                             bool
	movement_mode                     AuthoritativeMovementMode
	current_tick                      i64
...
pub fn (p &StartGamePacket) encode_payload(mut w serializer.Writer) {
	...
	types.write_experiments(mut w, p.experiments)
	...
	w.bool(p.is_nether_type)
	w.bool(p.has_force_experimental_gameplay)
	...
	w.bool(p.trial)
	w.write_varint32(i32(p.movement_mode))
	...
	for definition in p.item_definitions {
		...
		w.bool(definition.component_based)
	}
...
pub fn (mut p StartGamePacket) decode_payload(mut r serializer.Reader) ! {
	...
	mut gamerules := []types_291.GameRuleData{cap: serializer.prealloc(rule_count)}
	for _ in 0 .. rule_count {
		gamerules << types_291.GameRuleData.decode(mut r)!
	}
	p.gamerules = gamerules
	p.experiments = types.read_experiments(mut r)!
	...
	p.is_nether_type = r.bool()!
	p.has_force_experimental_gameplay = r.bool()!
	if p.has_force_experimental_gameplay {
	...
	p.trial = r.bool()!
	p.movement_mode = unsafe { AuthoritativeMovementMode(r.read_varint32()!) }
	p.current_tick = r.le_i64()!
	p.enchantment_seed = r.read_varint32()!
	block_count := r.read_count()!
	mut blocks := []BlockPropertyData{cap: serializer.prealloc(block_count)}
	...
	mut definitions := []ItemDefinitionEntry{cap: serializer.prealloc(definition_count)}
	for _ in 0 .. definition_count {
		definitions << ItemDefinitionEntry{
			identifier:      r.read_string()!
			runtime_id:      r.le_i16()!
			component_based: r.bool()!
		}
	}
	p.item_definitions = definitions
