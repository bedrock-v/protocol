...
pub mut:
	recipe_id  string
	base       types_554.ItemDescriptorWithCount
	addition   types_554.ItemDescriptorWithCount
	result     types_431.ItemData
	tag        string
	network_id u32
}
...
pub fn RecipeData.decode(mut r serializer.Reader) !RecipeData {
	...
	match recipe_type {
		...
		crafting_data_type_shapeless_chemistry {
			...
			t.ingredients = []types_554.ItemDescriptorWithCount{cap: serializer.prealloc(ingredient_count)}
			for _ in 0 .. ingredient_count {
				t.ingredients << types_554.ItemDescriptorWithCount.decode(mut r)!
			}
			result_count := r.read_count()!
			t.results = []types_431.ItemData{cap: serializer.prealloc(result_count)}
			for _ in 0 .. result_count {
				t.results << types_431.ItemData.decode_instance(mut r)!
			}
		...
		crafting_data_type_shaped, crafting_data_type_shaped_chemistry {
			...
			t.ingredients = []types_554.ItemDescriptorWithCount{cap: serializer.prealloc(input_count)}
		...
		crafting_data_type_furnace, crafting_data_type_furnace_data {
			...
			t.result = types_431.ItemData.decode_instance(mut r)!
			t.tag = r.read_string()!
			return t
		}
		...
		crafting_data_type_smithing_transform {
			return SmithingTransformRecipeData{
				...
				base:       types_554.ItemDescriptorWithCount.decode(mut r)!
				addition:   types_554.ItemDescriptorWithCount.decode(mut r)!
