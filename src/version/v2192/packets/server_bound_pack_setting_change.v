...
pub struct PackSettingStringArray {
pub mut:
	value []string
}

pub type PackSettingValue = PackSettingBool
	| PackSettingFloat
	| PackSettingString
	| PackSettingStringArray
...
pub fn (t PackSettingValue) encode(mut w serializer.Writer) {
	match t {
		...
		PackSettingStringArray {
			w.write_varuint32(3)
			w.write_varuint32(u32(t.value.len))
			for s in t.value {
				w.write_string(s)
			}
		}
