...
pub mut:
	...
	item_use_has_network_ids   bool
	item_use_actions           []types_407.InventoryActionData
...
pub fn (p &PlayerAuthInputPacket) encode_payload(mut w serializer.Writer) {
	...
	if p.input_data & input_flag_perform_item_interaction != 0 {
		...
		w.bool(p.item_use_has_network_ids)
		w.write_varuint32(u32(p.item_use_actions.len))
		for action in p.item_use_actions {
			action.encode(mut w, p.item_use_has_network_ids)
		}
