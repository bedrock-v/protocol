...
pub mut:
	...
	disabling_personas                  bool
	disabling_custom_skins              bool
	...
	trial                               bool
	authoritative_movement_mode         i32
	rewind_history_size                 i32
	server_authoritative_block_breaking bool
	current_tick                        i64
	enchantment_seed                    i32
	block_properties                    []BlockPropertyEntry
	...
	client_side_generation_enabled      bool
	block_network_ids_hashed            bool
...
fn (p &StartGamePacket) encode_level_settings(mut w serializer.Writer) {
	...
	p.xbl_broadcast_mode.encode(mut w)
	p.platform_broadcast_mode.encode(mut w)
	w.bool(p.commands_enabled)
	w.bool(p.texture_packs_required)
	w.write_varuint32(u32(p.gamerules.len))
	for gamerule in p.gamerules {
		gamerule.encode(mut w)
	}
	w.le_u32(u32(p.experiments.len))
	...
	w.bool(p.disabling_personas)
	w.bool(p.disabling_custom_skins)
	...
	if force := p.force_experimental_gameplay {
		w.bool(true)
		w.bool(force)
	} else {
		w.bool(false)
	...
	w.bool(p.disabling_player_interactions)
}
...
fn (mut p StartGamePacket) decode_level_settings(mut r serializer.Reader) ! {
	...
	p.xbl_broadcast_mode = GamePublishSetting.decode(mut r)!
	p.platform_broadcast_mode = GamePublishSetting.decode(mut r)!
	p.commands_enabled = r.bool()!
	p.texture_packs_required = r.bool()!
	gamerule_count := r.read_count()!
	p.gamerules = []types_440.GameRuleData{cap: serializer.prealloc(gamerule_count)}
	for _ in 0 .. gamerule_count {
		p.gamerules << types_440.GameRuleData.decode(mut r)!
	}
	experiment_count := int(r.le_u32()!)
	p.experiments = []types_419.ExperimentData{cap: serializer.prealloc(experiment_count)}
	for _ in 0 .. experiment_count {
		p.experiments << types_419.ExperimentData.decode(mut r)!
	}
	...
	p.disabling_personas = r.bool()!
	p.disabling_custom_skins = r.bool()!
	...
	p.disabling_player_interactions = r.bool()!
}
...
pub fn (p &StartGamePacket) encode_payload(mut w serializer.Writer) {
	...
	w.bool(p.trial)
	w.write_varint32(p.authoritative_movement_mode)
	...
	for block_property in p.block_properties {
		w.write_string(block_property.name)
		w.write_nbt_compound_root(block_property.properties)
	}
	w.write_varuint32(u32(p.item_definitions.len))
	for item_definition in p.item_definitions {
		w.write_string(item_definition.identifier)
		w.le_i16(item_definition.runtime_id)
		w.bool(item_definition.component_based)
	}
	...
	w.bool(p.client_side_generation_enabled)
	w.bool(p.block_network_ids_hashed)
	w.bool(p.server_auth_sound_enabled)
}
...
pub fn (mut p StartGamePacket) decode_payload(mut r serializer.Reader) ! {
	...
	p.trial = r.bool()!
	p.authoritative_movement_mode = r.read_varint32()!
	...
	p.block_properties = []BlockPropertyEntry{cap: serializer.prealloc(block_property_count)}
	for _ in 0 .. block_property_count {
		p.block_properties << BlockPropertyEntry{
			name:       r.read_string()!
			properties: r.read_nbt_compound_root()!
		}
	}
	item_definition_count := r.read_count()!
	p.item_definitions = []ItemDefinitionEntry{cap: serializer.prealloc(item_definition_count)}
	for _ in 0 .. item_definition_count {
		p.item_definitions << ItemDefinitionEntry{
	...
	p.client_side_generation_enabled = r.bool()!
	p.block_network_ids_hashed = r.bool()!
	p.server_auth_sound_enabled = r.bool()!
}
