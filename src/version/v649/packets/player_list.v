...
import protocol.version.v568.types as types_568
...
pub fn (mut p PlayerListPacket) decode_payload(mut r serializer.Reader) ! {
	...
	if p.action == .add {
		for _ in 0 .. count {
			...
			e.build_platform = enums.BuildPlatform.decode(mut r)!
			e.skin = types_568.SerializedSkin.decode(mut r)!
			e.teacher = r.bool()!
			e.host = r.bool()!
			e.sub_client = r.bool()!
			p.entries << e
