...
pub fn (p &CameraPacket) encode_payload(mut w serializer.Writer) {
	w.write_varint64(p.camera_unique_id)
	w.write_varint64(p.player_unique_id)
}

pub fn (mut p CameraPacket) decode_payload(mut r serializer.Reader) ! {
	p.camera_unique_id = r.read_varint64()!
	p.player_unique_id = r.read_varint64()!
}
