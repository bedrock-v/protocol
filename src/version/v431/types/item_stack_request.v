...
pub fn (t ItemStackRequestAction) encode(mut w serializer.Writer) {
	match t {
		TakeAction {
			enums_428.ItemStackRequestActionType.take.encode(mut w)
			w.u8(t.count)
			t.source.encode(mut w)
			t.destination.encode(mut w)
		}
		PlaceAction {
			enums_428.ItemStackRequestActionType.place.encode(mut w)
			w.u8(t.count)
			t.source.encode(mut w)
			t.destination.encode(mut w)
		}
		SwapAction {
			enums_428.ItemStackRequestActionType.swap.encode(mut w)
			t.source.encode(mut w)
			t.destination.encode(mut w)
		}
		DropAction {
			enums_428.ItemStackRequestActionType.drop.encode(mut w)
			w.u8(t.count)
			t.source.encode(mut w)
			w.bool(t.randomly)
		}
		DestroyAction {
			enums_428.ItemStackRequestActionType.destroy.encode(mut w)
			w.u8(t.count)
			t.source.encode(mut w)
		}
		ConsumeAction {
			enums_428.ItemStackRequestActionType.consume.encode(mut w)
			w.u8(t.count)
			t.source.encode(mut w)
		}
		CreateAction {
			enums_428.ItemStackRequestActionType.create.encode(mut w)
			w.u8(t.slot)
		}
		LabTableCombineAction {
			enums_428.ItemStackRequestActionType.lab_table_combine.encode(mut w)
		}
		BeaconPaymentAction {
			enums_428.ItemStackRequestActionType.beacon_payment.encode(mut w)
			w.write_varint32(t.primary_effect)
			w.write_varint32(t.secondary_effect)
		}
		MineBlockAction {
			enums_428.ItemStackRequestActionType.mine_block.encode(mut w)
			w.write_varint32(t.hotbar_slot)
			w.write_varint32(t.predicted_durability)
			w.write_varint32(t.stack_network_id)
		}
		CraftRecipeAction {
			enums_428.ItemStackRequestActionType.craft_recipe.encode(mut w)
			w.write_varuint32(t.recipe_network_id)
		}
		AutoCraftRecipeAction {
			enums_428.ItemStackRequestActionType.craft_recipe_auto.encode(mut w)
			w.write_varuint32(t.recipe_network_id)
		}
		CraftCreativeAction {
			enums_428.ItemStackRequestActionType.craft_creative.encode(mut w)
			w.write_varuint32(t.creative_item_network_id)
		}
		CraftRecipeOptionalAction {
			enums_428.ItemStackRequestActionType.craft_recipe_optional.encode(mut w)
			w.write_varuint32(t.recipe_network_id)
			w.le_i32(t.filtered_string_index)
		}
		CraftNonImplementedAction {
			enums_428.ItemStackRequestActionType.craft_non_implemented_deprecated.encode(mut w)
		}
		CraftResultsDeprecatedAction {
			enums_428.ItemStackRequestActionType.craft_results_deprecated.encode(mut w)
...
pub fn ItemStackRequestAction.decode(mut r serializer.Reader) !ItemStackRequestAction {
	action_type := enums_428.ItemStackRequestActionType.decode(mut r)!
	match action_type {
		.take {
			return TakeAction{
				count:       r.u8()!
